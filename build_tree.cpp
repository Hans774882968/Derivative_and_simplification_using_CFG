...
class Parser {
 private:
  ...
  int sl;
  Val cur;
  int pos;  // 解析表达式的需求下，指针不用回退，因此设为全局变量
 public:
  AST *rt;
  ...
  void advance() {
    ...
    if (isdigit(s[pos])) {
      int x = 0;
      for (; pos < sl && isdigit(s[pos]);) x = 10 * x + s[pos++] - '0';
      cur = Val(x);
    } else if (isalpha(s[pos]) || s[pos] == '_') {
      string x;
      for (; pos < sl && (isalpha(s[pos]) || isdigit(s[pos]) || s[pos] == '_');)
        x += s[pos++];
      cur = Val(x);
    } else
      cur = Val(s[pos++]);
...
struct Simplifier {
  ...
  bool hasNoAM(AST *u) {  // 没有加or减号节点
    if (u->val.typ == 1) {
      if (u->val.cval == '+' || u->val.cval == '-') return false;
    }
    for (auto v : u->son) {
  ...
  string simplify() { return res = dfs(rt); }
  string dfs(AST *u) {
    string res;
    if (u->val.typ == 0)
      return to_string(u->val.ival);
    else if (u->val.typ == 2)
      return u->val.var;
    if (u->val.cval == '+') {
      res = dfsAddSon(u);
